...
class UNETR_decoder(nn.Module):
    ...
    def forward(self, x_in, x, hidden_states_out):
        ...
        out_dict['project'] = self.projector(dec2)
        out_dict['project_map'] = F.interpolate(self.convds0(out), size=dec2.shape[2:], mode='trilinear',
                                           align_corners=False)
        out_dict['level3'] = F.interpolate(self.convds3(dec3), size=size, mode='trilinear', align_corners=False)
        out_dict['level2'] = F.interpolate(self.convds2(dec2), size=size, mode='trilinear', align_corners=False)
        out_dict['level1'] = F.interpolate(self.convds1(dec1), size=size, mode='trilinear', align_corners=False)
        out_dict['out'] = F.interpolate(self.convds0(out), size=size, mode='trilinear', align_corners=False)
