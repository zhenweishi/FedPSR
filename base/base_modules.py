...
class NegativeSamplingPixelContrastiveLoss(nn.Module):
    ...
    def forward(self, input, positive, negative, input_logits=None, negative_logits=None, input_seg=None, negative_seg=None):
        ...
        # 检查 norm_negative 是否是 MetaTensor 类型
        if isinstance(norm_negative, MetaTensor):
            # 如果是 MetaTensor，则转换为标准的 torch.Tensor
            norm_negative = norm_negative.as_tensor()
        ...
        # 检查 sampled_negative_indices 是否为 MetaTensor 类型
        if isinstance(sampled_negative_indices, MetaTensor):
            # 如果是 MetaTensor，则转换为标准的 torch.Tensor
            sampled_negative_indices = sampled_negative_indices.as_tensor()
        # print("666666666666666666666666666666666666666666", sampled_negative_indices.dtype)
        #
        # print("sampled_negative_indices:", sampled_negative_indices)
        # print("sampled_negative_indices", sampled_negative_indices.size())
        sampled_negative = norm_negative[sampled_negative_indices]  # K, B * N, C
        # print("sampled_negative:", sampled_negative.size())
